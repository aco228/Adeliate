...
namespace Adlite.IPInserter.Models
{
  public class CountryModel
  {
    ...
    public static List<CountryModel> LoadAll()
    {
      ...
      DirectContainer dc = Program.AdliteDatabase.LoadContainer("SELECT CountryID, TwoLetterIsoCode FROM Adlite.core.Country");
      ...
      foreach(DirectContainerRow row in dc.Rows)
      {
        ...
        DirectContainer dcmno = Program.AdliteDatabase.LoadContainer("SELECT MobileOperatorID, Name FROM Adlite.core.MobileOperator WHERE CountryID=" + newModel.ID);
        if(dcmno.HasValue && dcmno.RowsCount > 0)
          foreach(DirectContainerRow dcrMno in dcmno.Rows)
          {
            ...
            DirectContainer dcMcc = Program.AdliteDatabase.LoadContainer("SELECT MCC, MNC FROM Adlite.core.MobileOperatorCode WHERE MobileOperatorID=" + mnoModel.ID);
